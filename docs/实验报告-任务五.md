...
## 任务内容
- 在 Vue 组件中定义数据模型（data）。
- 实现至少 2 个交互功能（示例：显示/隐藏、待办事项添加与删除）。

## （1）程序代码（实现交互的 Vue 组件代码）
组件文件：`src/pages/InteractionLab.vue`
...
## （2）运行结果（交互效果截图）
请在浏览器打开 `http://localhost:5173/interaction`，完成以下交互后截图：  
- 点击按钮切换提示列表的显示/隐藏  
- 待办事项列表新增一条并删除一条  

截图路径（建议）：`docs/screenshots/interaction.png`（自行替换）
